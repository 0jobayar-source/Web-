...
app.secret_key = "GST_GOD_MODE_ULTRA" 
...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('nagad_personal', '01737803621')")
...
def login():
    if request.method == 'POST':
        ...
        if user and user['is_banned']:
            conn.close()
            return render_template('login.html', error="🚫 YOU ARE BANNED BY GST OWNER")
...
def dashboard():
    ...
    nagad_personal = conn.execute("SELECT value FROM settings WHERE key='nagad_personal'").fetchone()['value']
    
    conn.close()
    
    return render_template('dashboard.html', user=user, bots=bots, 
                           vip_price=vip_price, premium_price=premium_price, nagad_personal=nagad_personal)
...
def update_settings():
    ...
    nagad_personal = request.form.get('nagad_personal')
    ...
    conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('nagad_personal', ?)", (nagad_personal,))
    conn.commit()
    conn.close()
